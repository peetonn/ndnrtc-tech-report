...
The high-level concept of \ndnrtcName{} as a research experiment is as follows:  Through it, we investigate if ``real-time'' content publishing can be achieved as with most other NDN data dissemination applications, described as follows.

In this scenario, bidirectional communication is achieved by having each node participate as a publisher and consumer. The publisher 1) acquires and transforms media data, 2) names, packetizes and signs it, and 3) then passes it to an internal or external component that responds to Interests received from the ``black box'' of the NDN network (matched against available signed, named data chunks). The consumer issues Interests with appropriate names and selectors to that ``black box'' of the NDN network, at the rate necessary to achieve its objectives and be a good citizen of the network\footnote{While work on congestion control and defining proper behavior of such applications on the network is underway, it is future work relative to this paper.}--as informed by the performance of the network it observes in response to its requests. It reassembles and renders them.  
...
\item \textbf{Adaptive bitrate.} The library and namespace should support multiple bitrates for consumers to select from and lay the ground work for research on adaptive schemes in the future. 
...
There are two application roles in \ndnrtcName{}: producer and consumer.  In bidirectional communication, applications play both roles, but multi-party and one-to-many scenarios are achieved with the same abstraction.  In the application, the paradigm of real-time communication shifts from sender-driven approach of the IP network, where the producer writes data to the network, and the consumer reads it as it arrives, to receiver-driven, in which the producer publishes data on the network at its own pace, while the consumer requests data as needed and manages the relationship between outgoing interests, incoming data segments, and buffer fill. 
...
A primary design question is thus how the data should be named, so it can be  retrieved by the consumer with desired properties.  The \ndnrtcName{} namespace defines names for media (segmented video frames and bundled audio samples), error correction data, and metadata, as shown in Figure~\ref{fig:namespace}.  As there is no direct consumer-producer communication, the namespace is designed to efficiently support the type of fetching operations performed by the consumer introduced in Section~\ref{sec:arch} and detailed in Section~\ref{sec:consumer}. 
...
The receiver-driven architecture of \ndnrtcName{} and our experimental approach's deliberate avoidance of explicit consumer-producer synchronization (to explore network scaling support) have suggested the importance of providing sufficient meta information from the producer. The most critical, and potentially useful for a variety of applications and services, is provided in the namespace, as described above.  Other information is provided in the data objects themselves. Such separation is currently experimental and provides some benefits such as, for example, being able to quickly retrieve total number of segments in the frame without the need for content decoding or knowledge of the exact payload format.
