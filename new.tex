\documentclass[10pt]{icn/sig-alternate-10pt} % {proc}
...
%\author[1]{Peter Gusev \href{mailto:peter@remap.ucla.edu}{peter@remap.ucla.edu}}
%\author[1]{Jeff Burke \href{mailto:jburke@remap.ucla.edu}{jburke@remap.ucla.edu}}
%\affil[1]{UCLA REMAP}
\author{Peter Gusev \\ \href{mailto:peter@remap.ucla.edu}{\texttt{peter@remap.ucla.edu}} 
\and Jeff Burke \\ \href{mailto:jburke@remap.ucla.edu}{\texttt{jburke@remap.ucla.edu}}}
...
\section{Goals}
\label{sec:goals}
Initial goals of the \ndnrtcName{} project are to provide low-latency audio/video multi-party conferencing application which can be used by NDN project team members across existing NDN testbed. It can be also used as a traffic generator application for future researches in NDN routing algorithms, congestion control and general testing of the network architecture ideas.

\paragraph{Low-latency (150-300ms) audio/video communication}
Library should be capable to maintain similar sub-second latency communication for audio and video as in dirver applications like Skype, WebEX, Goolg Hangouts.

\paragraph{Multi-party conferencing}
Publishing and fetching several media streams simultaneously should not require significant computational resources from the user and should maintain the same latency as in one-to-one conferencing case.

\paragraph{Passive consumer}
There should be no explicit negotiation or any coordination between active conference members as it may limit scalability and flexibility of use.

\paragraph{Content verification}
Library should provide content verification using existing NDN signature capabilities. 
...
\label{sec:imp}
\ndnrtcName{} is implemented as a library written in C++. 
...
\ndnrtcName{} exploits some functionality from several third-party libraries it is linked against with:
\begin{itemize} 
\item NDN-CPP \cite{ndnccl} is used for NDN connectivity;
\item WebRTC framework \cite{webrtc} is utilized in two ways:
\begin{itemize}
\item incorporation of the existing video codec
\item full incorporation of the existing WebRTC audio pipeline, including echo cancellation
\end{itemize}
\item OpenFec \cite{openfec} library is utilized for forwad error correction support.
\end{itemize}

Apart from the library, first desktop NDN videconferencing application NdnCon \cite{ndncon} was implemented atop \ndnrtcName{}. It provides convenient UI for publishing and fetching media streams, text chat and organizing multi-party audio/video conferences.
...
An elegant way to offload producer from publishing multiple copies of the same video stream in different bandwidths is to utilize Scalable Video Coding. By reflecting SVC layers in the namespace, consumer can have more freedom for adapting media streams to the current network. This opportunity should be explored and added in a future versions.

Current \ndnrtcName{} design supports basic content signing and verification. However, several basic security features are not yet implemented:

\begin{itemize}
\item signatures consistency checks for successive media packets;
\item unknown but verified publishers trust; 
\item media data encryption;
\item consumer access control.
\end{itemize}

Currently, any media streams that are published over the NDN network can be fetched without publisher's knowledge which keeps conferencing far from privacy and introduces analogy of a "video Twitter" with ungranted access to the published data. While this might seem desirable and useful in some cases, more work is needed to support conventional "conferencing" paradigm with privacy, verification and access control.
